...
\newunicodechar{θ}{{\greekfont θ}}
\newunicodechar{σ}{{\greekfont σ}}
...
\begin{itemize}
    ...
    \item \textbf sex    - male and female
    \item \textbf bmi    - body mass index
    \item \textbf bp     - average blood pressure
    \item \textbf s1     - TC: total serum cholesterol
    \item \textbf s2     - LDL: low-density lipoproteins
    \item \textbf s3     - HDL: high-density lipoproteins
    \item \textbf s4     - TCH: total cholesterol / HDL
    \item \textbf s5     - LTG: possibly log of serum triglycerides level
    \item \textbf s6     - GLU: blood sugar level
    \item \textbf target - measure of disease progression one year after baseline
\end{itemize}
...
@
\subsection*{MLE Data Analysis}
With the assumption that the data is of a normal distribution and follows linear regression beta0 = 21.86, indicates that this is your progression if your BMI was 0, this is unrealistic as no one has this, the slope being labelled beta1 = 0.31 indicates every BMI increment of 1 would increase progression by 0.31 showing a small positive association which is reasonable, and standard deviation being 4102.63 is unrealistically high implying that BMI alone is not enough to determine the progression of diabetes based on the model or there is outlier variance that is just not explained by BMI. This is reasonable given the data is 10 measured variables and you would assume that the other 9 are not all redundant and cannot be predicted by BMI alone even though there is some correlation.
...
cat("p-value:", p_value, "\n")
@
\subsection*{Likelihood Ratio Test Analysis}
...
\subsection{Ranking the variables based on the correlation coefficient}
<<echo=TRUE>>=
...
In conclusion, the comparison between the Newton-Raphson and bootstrap methods highlights the importance of using multiple statistical approaches when analyzing complex datasets. While the Newton-Raphson method provided consistent but potentially oversimplified estimates, the bootstrap method offers a more detailed and possibly more accurate representation of the data's statistical properties. This comparison not only enhances our understanding of the relationship between blood pressure and diabetes progression but also serves as a reminder of the nuances involved in statistical estimation and the value of non-parametric techniques in handling normalized data.
